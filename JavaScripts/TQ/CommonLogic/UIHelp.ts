export namespace UIHelp {
    ...
    /**让一个按钮长按显示详情 */
    export function setBtnDes(btn: UI.Button | UI.StaleButton, callback: () => void, Released: () => void, check: number = 500) {
        const guid = btn.guid;
    ...
    /**设置按钮的双击事件 */
    export function doubleClick(btn: UI.Button | UI.StaleButton, f: () => void, check: number = 500) {
        const guid = btn.guid;
    ...
    export function setBtnDrag(btn: UI.Button | UI.StaleButton, f: (x: number, y: number) => void, clearOldAction: boolean = true) {
        let dragID = null;
        if (clearOldAction) {
            btn.onPressed.clear();
        ...
        btn.onPressed.add(() => {
            ...
            let offset = Type.Vector2.zero
            dragID = setInterval(() => {
                let cursorPos = UI.getMousePositionOnViewport();//鼠标位置
                offset = Type.Vector2.subtract(cursorPos, beforeCursorPos);
                btn.position = Type.Vector2.add(offset, btn.position);
